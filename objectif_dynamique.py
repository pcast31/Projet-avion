...
def carre_max(X,i,j,i_max,j_max):
    if sum([X[i2][j2] for j2 in range(j,j_max) for i2 in range(i,i_max)])==0 and(i_max-i)*(j_max-j)<=16:
...
def objectif_inter(model, X, ind,coef = [0,1,1],a=5, b = 1):
    (N,P,K)=np.shape(X)
    model.setObjective(a*espacement(ind,X)/(N*P-K) + correspondance(model, X, ind, b) + bonus_seul(model,X,ind,coef), GRB.MAXIMIZE)
